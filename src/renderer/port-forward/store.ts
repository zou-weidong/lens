...
import { PortForwardItem } from "./item";
import { waitUntilFree } from "tcp-port-used";
import type { LensLogger } from "../../common/logger";
import type { JsonApiData, JsonApiHandler } from "../../common/k8s-api/json-api";
import type { PortForwardingErrorNotification } from "./error-notification.injectable";
import type { StorageLayer } from "../utils/storage/create.injectable";

interface Dependencies {
  readonly storage: StorageLayer<ForwardedPort[] | undefined>;
  readonly apiBase: JsonApiHandler<JsonApiData>;
...
export class PortForwardStore extends ItemStore<PortForwardItem> {
  ...
  private async init() {
    ...
    if (Array.isArray(savedPortForwards) && savedPortForwards.length > 0) {
      this.dependencies.logger.info("starting saved port-forwards");
      ...
      const hasErrorResult = results.findIndex(result => result.status === "rejected" || result.value.status === "Disabled") >= 0;

      if (hasErrorResult) {
        this.dependencies.portForwardingErrorNotification("One or more port-forwards could not be started");
      }
    }
  }
  ...
  remove = action(async (portForward: ForwardedPort) => {
    ...
    if (!pf) {
      ...
      this.dependencies.logger.warn(
        `Error getting port-forward: ${error}`,
        portForward,
      );
    ...
    try {
      ...
      if (pf.status === "Active") {
        this.dependencies.logger.warn(
          `Error removing port-forward: ${error}`,
          portForward,
        );
      }
    }
  ...
  stop = action(async (portForward: ForwardedPort) => {
    ...
    if (!pf) {
      this.dependencies.logger.warn(
        "Error getting port-forward: port-forward not found",
        portForward,
      );
    ...
    try {
      ...
      this.dependencies.logger.warn(
        `Error stopping active port-forward: ${error}`,
        portForward,
      );
      throw error;
    }
  ...
  start = action(async (portForward: ForwardedPort): Promise<ForwardedPort> => {
    ...
    try {
      response = await this.dependencies.apiBase.post<PortForwardResult>(
      ...
      if (
        ...
        this.dependencies.logger.warn(
          `specified ${pf.forwardPort}, got ${response.port}`,
        );
      }
      ...
      this.dependencies.logger.warn(
        `Error starting port-forward: ${error}`,
        pf,
      );
      pf.status = "Disabled";
    }
  ...
  getPortForward = async (
    ...
    try {
      ...
      pf = await this.getActivePortForward(portForward);

      if (pf.forwardPort && pf.forwardPort !== portForward.forwardPort) {
        this.dependencies.logger.warn(
          `local port, expected ${pf.forwardPort}, got ${portForward.forwardPort}`,
        );
      }
    } catch (error) {
      // port is not active
  ...
  private async getActivePortForward(portForward: ForwardedPort): Promise<ForwardedPort> {
    const { port, forwardPort } = portForward;
    let response: PortForwardResult;

    try {
      response = await this.dependencies.apiBase.get<PortForwardResult>(`/pods/port-forward/${portForward.namespace}/${portForward.kind}/${portForward.name}`, { query: { port, forwardPort }});
    } catch (error) {
      this.dependencies.logger.warn(`Error getting active port-forward: ${error}`, portForward);
    }
