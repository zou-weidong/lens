 ...
 */
const detailsSelflinkPageParamInjectable = getInjectable({
  id: "details-selflink-page-param",
  instantiate: (di) => {
    const createPageParam = di.inject(createPageParamInjectable);

    return createPageParam<string>({
      name: "kube-details-self-link",
    });
  },
});

export default detailsSelflinkPageParamInjectable;
