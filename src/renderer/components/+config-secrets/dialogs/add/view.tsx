...
export interface AddSecretDialogProps extends Omit<DialogProps, "isOpen" | "close"> {
}
...
class NonInjectedAddSecretDialog extends React.Component<AddSecretDialogProps & Dependencies> {
  ...
  createSecret = async () => {
    ...
    const { showDetails, close, errorNotification, secretApi } = this.props;
    ...
    try {
      showDetails(await secretApi.create({ namespace, name }, secret));
      close();
    } catch (err) {
...
export const AddSecretDialog = withInjectables<Dependencies, AddSecretDialogProps>(NonInjectedAddSecretDialog, {
  getProps: (di, props) => ({
    ...props,
    close: di.inject(closeAddSecretDialogInjectable),
    errorNotification: di.inject(errorNotificationInjectable),
    isOpen: di.inject(isAddSecretDialogOpenInjectable),
