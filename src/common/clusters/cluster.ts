...
import { loadConfigFromFile, loadConfigFromFileSync, validateKubeConfig } from "../k8s/helpers";
import type { KubeApiResource, KubeResource } from "../k8s/resources";
import { apiResourceRecord, apiResources } from "../k8s/resources";
...
import type { ClusterDetectionResult } from "../../main/clusters/metadata/cluster-detector";
import type { LensLogger } from "../logger";
import type { ConnectionUpdate } from "../ipc/cluster/connection-update.token";
...
export interface ClusterDependencies {
  ...
  emitConnectionUpdate: ConnectionUpdate;
  detectMetadataForCluster: (cluster: Cluster) => Promise<ClusterMetadata>;
  detectSpecificMetadataForCluster: (cluster: Cluster, kind: string) => Promise<ClusterDetectionResult>;
}
...
export class Cluster implements ClusterModel, ClusterState {
  ...
  protected async getConnectionStatus(): Promise<ClusterStatus> {
    try {
      const versionData = await this.dependencies.detectSpecificMetadataForCluster(this, ClusterMetadataKey.VERSION);
  ...
  pushState(state = this.getState()) {
    // this.dependencies.logger.debug(`[CLUSTER]: push-state`, state);
  ...
  broadcastConnectUpdate(message: string, isError = false): void {
    ...
    this.dependencies.emitConnectionUpdate(this.id, update);
  }
